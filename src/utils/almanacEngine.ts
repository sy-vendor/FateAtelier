export interface AlmanacResult {
  ...
  yi: string[]
  ji: string[]
  shichenJixiong: Array<{ shichen: string; jixiong: string }>
  wuxing: string
  jishenFangwei: string
  xiongshenFangwei: string
}
...
// 生成黄历（基于天干地支规则，非随机）
export function generateAlmanac(date: Date): AlmanacResult {
  const year = date.getFullYear()
  const month = date.getMonth() + 1
  const day = date.getDate()
...
export function shichenTagClass(jixiong: string): string {
  if (jixiong === '吉') return 'tag tag--good'
  if (jixiong === '凶') return 'tag tag--bad'
