...
for (const [slug, title, description, intro, question, answer, related = []] of pages) {
  const url = `${origin}/${slug}`
  const fullTitle = `${title} | 命运工坊`
  const relatedHtml = related.length
    ? `<h2>相关功能</h2><ul>${related.map(([href, label]) => `<li><a href="${href}">${escapeHtml(label)}</a></li>`).join('')}</ul>`
    : ''
  const schema = {
    '@context': 'https://schema.org',
    '@graph': [
  ...
  const crawlable = `<div id="root"><main class="seo-entry"><h1>${title}</h1><p>${description}</p><h2>功能介绍</h2><p>${intro}</p><h2>常见问题</h2><h3>${question}</h3><p>${answer}</p>${relatedHtml}<p><a href="/">返回命运工坊</a></p></main></div>`
  const html = template
...
const sitemapEntries = [
  ...
  ...detailUrls.map((url) => ({
    loc: url,
    changefreq: url.includes('/card/') || url.includes('/symbol/') || url.includes('/stick/') ? 'monthly' : 'weekly',
    priority: url.endsWith('/cards') || url.endsWith('/symbols') || url.endsWith('/sticks') ? '0.6' : '0.5',
  })),
]
const sitemapXml = [
  '<?xml version="1.0" encoding="UTF-8"?>',
  '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">',
  ...sitemapEntries.map(({ loc, changefreq, priority }) => [
    '  <url>',
    `    <loc>${loc}</loc>`,
...
console.log(`Built ${pages.length} feature pages, ${detailUrls.length} long-tail SEO pages, sitemap ${sitemapEntries.length} urls`)
